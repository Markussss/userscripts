// ==UserScript==
// @name     Fix problems in Nordea Investor
// @version  1
// @grant    none
...
function removeIgnoredNames() {
  Array.from(document.querySelectorAll('li[data-name=MYINSTRUMENTS] tr'))
    .filter((tr) => ignoredNames.reduce((ignore, name) => ignore || tr.textContent.includes(name), false))
  	.forEach((tr) => tr.remove());
};

function getRowValue(instrumentRow) {
  return parseFloat(
    instrumentRow.children[instrumentRow.children.length - 1].textContent.replace(/\s/g, '').replace(',', '.'),
    10
  );
}

function calculateNewTotal() {
  const valueRow = Array.from(document.querySelectorAll('li[data-name=MYINSTRUMENTS] tr'))
  	.find((tr) => tr.textContent.includes('Total markedsverdi'));
  
  if (!valueRow) return;
  
  const totalValue = Array.from(document.querySelectorAll('li[data-name=MYINSTRUMENTS] tbody tr'))
  	.map(getRowValue)
  	.reduce((sum, value) => sum + value, 0);
  valueRow.children[valueRow.children.length - 1].children[0].innerHTML = currencyFormatter.format(totalValue)
}

function getLatestPrice(instrumentRow) {
  const instrumentName = instrumentRow.querySelector('.table-naming').textContent.trim();
  let latestPriceNode = Array.from(document.querySelectorAll('li[data-name=MY_WATCHLIST] tbody tr'))
  	.find((row) => row.textContent.includes(instrumentName));
  
	if (latestPriceNode.querySelectorAll('[title="Realtidskurser"]').length > 0) {
  	latestPriceNode = latestPriceNode.querySelectorAll('[title="Realtidskurser"]')
  } else {
	  latestPriceNode = latestPriceNode.querySelectorAll('[title="Børsen er stengt"]');
  }
...
function addLatestPriceToInstruments() {
  const headers = document.querySelector('li[data-name=MYINSTRUMENTS] tr.headers');
  if (headers && headers.children.length === 7) {
	  const footers = document.querySelector('li[data-name=MYINSTRUMENTS] tfoot tr');
  ...
  const instruments = Array.from(document.querySelectorAll('li[data-name=MYINSTRUMENTS] tbody tr'));
  instruments.forEach((instrument) => {
    ...
    if (update) {
      update.children[0].innerHTML = getLatestPrice(instrument).textContent;
  		...
  		instrument.insertBefore(getLatestPrice(instrument), instrument.children[5]);
    }
  });
...
function getInstrumentNameAndTicker() {
  return Array.from(document.querySelectorAll('[data-name="COMPANY_HIGHLIGHTS"] td'))
...
function addLinksToInstrumentDetails() {
  const links = {
  	NorskBulls: ({ ticker }) => `https://www.norskbulls.com/SignalPage.aspx?lang=en&Ticker=${getTicker(ticker)}.OL`,
    Shareville: ({ name }) => `https://www.shareville.no/aksjer/${getName(name.toLowerCase().replace(/\s/g, '-'))}`,
  };
  ...
  if (document.querySelector('body.INSTRUMENT_DETAILS')) {
    ...
    Object.entries(links).forEach(([ name, linkFunction ]) => {
      ...
      button.href = linkFunction({ name: instrumentName, ticker: instrumentTicker });
...
function removeTempWidgetListBullet() {
  ...
  document.querySelector('li[data-name="TEMP_WIDGET"]').style.listStyleType = 'none';
}
...
(() => {
    ...
    observer.disconnect();
    domChanged();
    setTimeout(() => observer.observe(html, { attributes: true, childList: true, subtree: true }), 500);
  });
